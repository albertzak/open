    ...
    for (const range of view.visibleRanges) {
        syntaxTree(view.state).iterate({
            ...
            enter: ({ type, from, to }) => {
              ...
              // console.log(type.name, view.state.doc.sliceString(from, to))
                if (type.name === 'StringContent') {
                    const str = view.state.doc.sliceString(from, to);

                    const s = str.toString() || ''
                    const re = /(\#[a-f0-9]{2,8})|(rgba?\(.*\))|(hsla?\(.*\))/ig
                    let m = null

                    while ((m = re.exec(s)) != null) {
                      const match = m[0]
                      const matchFrom = from + m.index
                      const matchTo = from + m.index + match.length


                      //console.log(match, matchFrom, matchTo)
                ...
                } else if (type.name === 'CallExpression') {
...
const colorPickerViewPlugin = /*@__PURE__*/ViewPlugin.fromClass(class ColorPickerViewPlugin {
    ...
    update(update) {
      ...
      // TODO: better way to disable update if picker is currently in use
        // console.log(this.previousDocLength, update.state.doc.length)

        if (
            (update.docChanged || update.viewportChanged) &&
            (this.previousDocLength !== update.state.doc.length)
         ) {
    ...
    eventHandlers: {
        input: (e, view) => {
            ...
            return undefined; // otherwise color picker stops after first picking
        },
    },
});
